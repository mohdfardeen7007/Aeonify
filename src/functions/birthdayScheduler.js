...
async function sendBirthdayWish(Aeonify, birthday) {
  try {
    ...
    if (birthday.wishInGroups) {
      if (birthday.wishInAllGroups) {
        const groups = await Aeonify.groupFetchAllParticipating();
        const targetGroups = [];
        ...
        for (const { groupId, groupName } of targetGroups) {
          try {
            await Aeonify.sendMessage(groupId, {
        ...
        for (const group of birthday.groups) {
          try {
            const groupMetadata = await Aeonify.groupMetadata(group.groupId);
            const participants = groupMetadata.participants || [];
            const isUserInGroup = participants.some(p => p.id === birthday.userId);
            
            if (isUserInGroup) {
              await Aeonify.sendMessage(group.groupId, {
                text: birthdayMessage
              });
    ...
    if (birthday.wishInPrivate) {
      try {
        await Aeonify.sendMessage(birthday.userId, {
...
async function sendBirthdayReminders(Aeonify) {
  try {
    const today = new Date();
    ...
    for (const birthday of birthdays) {
      try {
        ...
        await Aeonify.sendMessage(config.ownerNumber, {
...
async function prepareBirthdayWishes(Aeonify) {
  try {
    const today = new Date();
    const birthdays = await Birthday.find({
      $expr: {
...
async function sendPreparedBirthdayWishes(Aeonify) {
  try {
    const birthdays = await prepareBirthdayWishes(Aeonify);
    
    for (const birthday of birthdays) {
      await sendBirthdayWish(Aeonify, birthday);
...
async function retryFailedWishes(Aeonify) {
  try {
    const today = new Date();
    const birthdays = await Birthday.find({
      $expr: {
...
async function retryFailedReminders(Aeonify) {
  try {
    const today = new Date();
...
async function checkBirthdays(Aeonify) {
  try {
    console.log('Checking birthdays...');
    
    await sendBirthdayReminders(Aeonify);
    await sendPreparedBirthdayWishes(Aeonify);
...
export function startBirthdayScheduler(Aeonify) {
  ...
  birthdayScheduler = setInterval(async () => {
    await checkBirthdays(Aeonify);
  }, 60 * 60 * 1000);

  retryScheduler = setInterval(async () => {
    await retryFailedWishes(Aeonify);
    await retryFailedReminders(Aeonify);
  }, 2 * 60 * 60 * 1000);
...
export async function manualBirthdayCheck(Aeonify) {
  console.log('Manual birthday check triggered');
  await checkBirthdays(Aeonify);
}

export async function manualRetryFailed(Aeonify) {
