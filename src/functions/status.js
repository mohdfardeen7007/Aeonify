...
function getCurrentStatus() {
  ...
  switch (mode) {
    ...
    case STATUS_MODES.UPTIME_ONLY: {
      ...
      return `Aeonify • Personal Edition • Uptime: ${uptimeFormatted}`;
    }
...
function updateStatus(Aeonify) {
  try {
    const status = getCurrentStatus();
    if (status) {
      Aeonify.updateProfileStatus(status);
      const mode = getStatusMode();
...
function startStatusUpdater(Aeonify) {
  if (!global.startTime) {
