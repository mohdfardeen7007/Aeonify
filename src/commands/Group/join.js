const joinCommand = {
  ...
  run: async ({ Aeonify, messages: m, jid, args, isOwner, isSelf, react }) => {
    ...
    try {
      ...
      if (isOwner || isSelf) {
        try {
          const result = await Aeonify.groupAcceptInvite(inviteCode);
          await react && react("✅");
        ...
        try {
          const queryResult = await Aeonify.query({
            tag: "iq",
            attrs: {
              type: "get",
              xmlns: "w:g2",
