...
const tagallCommand = {
  ...
  run: async (ctx) => {
    const { Aeonify, jid, messages: m, groupMetadata, text } = ctx;
    ...
    .catch(async (err) => {
      ...
      await Aeonify.sendMessage(jid, { text: "*Failed to tag members!* Please try again later." });
    });
  }
};
