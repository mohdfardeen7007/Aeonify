...
const groupInfoCommand = {
  ...
  run: async (ctx) => {
    ...
    try {
      await Aeonify.sendMessage(jid, { react: { text: "ℹ️", key: m.key }});

      const totalMembers = groupMetadata.participants.length;
      const admins = groupMetadata.participants.filter(p => p.admin).length;
      const members = totalMembers - admins;

      const settings = await Aeonify.groupMetadata(jid);
      ...
      .catch(async (err) => {
        ...
        await Aeonify.sendMessage(jid, { text: "*Failed to fetch group info!* Please try again later." });
      });
