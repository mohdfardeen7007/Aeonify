...
const afkCommand = {
  ...
  run: async (context) => {
    try {
      const { Aeonify, jid, messages, args, reply, sender, isOwner, senderName } = context;
...
async function handleSettings(context, args) {
  ...
  if (!args.length) {
    ...
    const settingsList = `*AFK Settings* ⚙️\n\n` +
      `🔔 *Notifications:*\n` +
  ...
  if (option === 'custom') {
    ...
    await AFK.findOneAndUpdate(
      ...
      { 'settings.customReply': customReply }
    );
...
export const handleAFKMentions = async (context) => {
  try {
    const { Aeonify, jid, messages, sender, isOwner, senderName, isGroup, groupName } = context;
    ...
    for (const mentionedUser of mentionedUsers) {
      ...
      if (afkStatus) {
        ...
        if (settings.autoReply) {
          await Aeonify.sendMessage(jid, { text: replyMessage }, { quoted: messages });
        }
        ...
        if (shouldNotify) {
          ...
          await Aeonify.sendMessage(mentionedUser, { text: dmNotice });
        }
      }
    }
  } catch (error) {
