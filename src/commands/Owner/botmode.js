...
export default {
  ...
  ownerOnly: true,
  cooldown: 5,
  run: async ({ Aeonify, reply, args, isOwner, isSelf }) => {
    try {
      if (!isOwner && !isSelf) {
      ...
      switch (action.toLowerCase()) {
        ...
        case 'grouponly':
          ...
          return reply(`✅ *Group Only Mode ${newMode.groupOnly ? 'Enabled' : 'Disabled'}*\n` +
            ...
            `*Note:* ${newMode.groupOnly ? 
              'The bot will ignore all private messages.' : 
              'The bot will work in both groups and private chats.'}`);

        case 'info':
          const info = `*🤖 Bot Mode Info*\n\n` +
            `*Current Mode:* ${mode.toUpperCase()}\n` +
            `*Group Only:* ${groupOnly ? '✅' : '❌'}\n` +
