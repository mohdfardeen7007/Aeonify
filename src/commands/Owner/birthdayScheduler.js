import { startBirthdayScheduler, stopBirthdayScheduler, manualBirthdayCheck } from '../../functions/birthdayScheduler.js';
...
const birthdaySchedulerCommand = {
  ...
  run: async ({ Aeonify, jid, message, args, sender, reply, isGroup, utils }) => {
    try {
      ...
      switch (action) {
        ...
        case 'check': {
          ...
          await manualBirthdayCheck(Aeonify);
        ...
        case 'status': {
          ...
          const statusMessage = `*🎂 Birthday Scheduler Status*\n\n` +
            `*Status:* ${status}\n` +
            `*Schedule:*\n` +
