...
const botProfileCommand = {
  ...
  run: async (ctx) => {
    const { Aeonify, jid, args, isOwner, isGroup } = ctx;
    try {
      if (isGroup) {
        if (!(await requireGroupPermissions(ctx, { admin: true, botAdmin: true }))) return;
      }
      if (!isOwner) {
        return await Aeonify.sendMessage(jid, { text: "*This command can only be used by the bot owner!*" });
      }
      ...
      if (args[0]?.toLowerCase() === 'about') {
        ...
        if (changed) {
          await botMode.save();
          return await Aeonify.sendMessage(jid, { text: msg.trim() });
        }
      ...
      if (args[0]?.toLowerCase() === 'add') {
        ...
        if (!argStr) {
          return await Aeonify.sendMessage(jid, { text: `*Usage:*\n!setbot add [your status message]` });
        }
        const { addStatusMessage } = await import('../../functions/status.js');
        const added = addStatusMessage(argStr);
        if (added) {
          return await Aeonify.sendMessage(jid, { text: `✅ *Status message added:* ${argStr}` });
        } else {
          return await Aeonify.sendMessage(jid, { text: `*That status message already exists or is invalid.*` });
        }
      }
      ...
      if (args[0]?.toLowerCase() === 'remove') {
        ...
        if (removed) {
          return await Aeonify.sendMessage(jid, { text: `✅ *Status message removed:* ${argStr}` });
        } else {
          return await Aeonify.sendMessage(jid, { text: `*Could not remove. Either it doesn't exist or it's a default message.*` });
        }
      }
...
${messages.map((msg, i) => `${i + 1}. ${msg}`).join('\n')}`
      ...
      if (args[0]?.toLowerCase() === 'reset') {
        ...
        resetStatusMessages();
        return await Aeonify.sendMessage(jid, { text: `✅ *Status messages reset to default!*` });
      }
