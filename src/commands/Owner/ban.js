  ...
  run: async ({ Aeonify, message, args, reply, sender, isOwner, messages: m }) => {
    ...
    try {
      ...
      if (!action || !isSubCommand) {
        return await handleBanUser({ Aeonify, message, args, reply, sender, sessionId, m });
      }
      ...
      switch (action.toLowerCase()) {
        ...
        case 'info':
          return await handleBanInfo({ Aeonify, message, args: restArgs, reply, sender, sessionId, m });
...
async function handleBanUser({ Aeonify, message, args, reply, sender, sessionId, m }) {
  ...
  try {
    ...
    if (duration !== 'permanent') {
      ...
      switch (unit) {
        case 'm': durationMinutes = value; break;
        case 'h': durationMinutes = value * 60; break;
        case 'd': durationMinutes = value * 1440; break;
        case 'w': durationMinutes = value * 10080; break;
        case 'y': durationMinutes = value * 525600; break;
      }
      banType = 'temporary';
    ...
    const updatedBan = await UserBan.findOneAndUpdate(
      { sessionId: sessionId, userId: targetJid },
      { $set: banData },
      { new: true, upsert: true, setDefaultsOnInsert: true }
    );
    
    const expiryText = updatedBan.expiresAt ? 
      `*Expires:* ${new Date(updatedBan.expiresAt).toLocaleString()}` : 
...
async function handleBanInfo({ Aeonify, message, args, reply, sender, sessionId, m }) {
  const { targetNumber, targetJid } = extractTargetUserUniversal({ m, args, message });
...
async function handleBanStats({ reply, sessionId }) {
  ...
  const statsText = `*📊 Ban Statistics*\n\n` +
