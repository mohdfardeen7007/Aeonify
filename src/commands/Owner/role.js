...
export default {
  name: 'role',
  aliases: ['roles', 'addrole', 'delrole', 'removerole', 'roleadd', 'roledel', 'roles'],
  ...
  run: async ({ Aeonify, message, args, reply, sender, isOwner, messages: m }) => {
    ...
    try {
      switch (action.toLowerCase()) {
        case 'add':
          return await handleRoleAdd({ Aeonify, message, args: restArgs, reply, sender, sessionId, m });
        
        case 'remove':
        case 'delete':
        case 'del':
          return await handleRoleRemove({ Aeonify, message, args: restArgs, reply, sender, sessionId, m });
        
        case 'list':
          return await handleRoleList({ reply, sessionId });
        
        case 'info':
          return await handleRoleInfo({ Aeonify, message, args: restArgs, reply, sender, sessionId, m });
...
// Handle role add
async function handleRoleAdd({ Aeonify, message, args, reply, sender, sessionId, m }) {
...
// Handle role remove
async function handleRoleRemove({ Aeonify, message, args, reply, sender, sessionId, m }) {
  const { targetNumber, targetJid } = extractTargetUserUniversal({ m, args, message });
...
// Handle role info
async function handleRoleInfo({ Aeonify, message, args, reply, sender, sessionId, m }) {
  const { targetNumber, targetJid } = extractTargetUserUniversal({ m, args, message });
...
async function handleRoleOwners({ reply, sessionId }) {
  ...
  const ownersList = `*👑 Owner List*\n\n` +
    owners.map((o, i) => 
      `${i + 1}. *${o.number}*\n` +
      `   Added: ${new Date(o.addedAt).toLocaleString()}`
    ).join('\n\n');
