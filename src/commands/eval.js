...
const evalCommand = {
  ...
  run: async ({ client, message, senderNumber, args, from }) => {
    if (!config.owner.number.includes(senderNumber)) {
      return await client.sendMessage(from, { text: "⛔ Access denied. Owner only." }, { quoted: message });
    }

    if (!args.length) {
      return await client.sendMessage(from, { text: "❗ Usage: !eval <JavaScript code>" }, { quoted: message });
    }
    ...
    const sendMessage = async (jid, content, options = {}) => {
      try {
        ...
        return await client.sendMessage(from, { text: `sendMessage error: ${err.message}` }, { quoted: message });
      }
    };
    ...
    try {
      ...
      await client.sendMessage(from, { text: errorMsg }, { quoted: message });
    }
  },
};
