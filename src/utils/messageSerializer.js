 ...
 * @param {Object} Aeonify - Aeonify instance
 * @param {Object} message - Message object
 * @param {Object} store - Message store for loading referenced messages
 * @returns {Object} Serialized message
 */
export const serializeMessage = async (Aeonify, message, store = null) => {
    try {
        // Basic validation
        ...
        const jidOwner = jidNormalizedUser(Aeonify?.user?.id);
        const lidOwner = jidNormalizedUser(Aeonify?.user?.lid);
            ...
            m.quoted = {
                ...
                download: async () => {
                    ...
                    try {
                        const buffer = await downloadMediaMessage(
                            ...
                            {},
                            {
                                reuploadRequest: Aeonify.updateMediaMessage,
                            }
                        );
                        return buffer;
                    } catch (error) {
                ...
                forward: async (jid, options = {}) => {
                    try {
                        ...
                        return await Aeonify.sendMessage(
                            jid ?? m.jid ?? '',
                            { forward: forwardMsg },
        ...
        // Add message methods
        m.reply = async (text) => {
            return await Aeonify.sendMessage(m.key.remoteJid ?? '', { text }, { quoted: message });
        };

        m.send = async (text) => {
            return await Aeonify.sendMessage(m.key.remoteJid ?? '', { text });
        };

        m.react = async (emoji) => {
            return await Aeonify.sendMessage(m.jid ?? '', {
                react: {
                    text: emoji,
                    key: m.key,
                },
